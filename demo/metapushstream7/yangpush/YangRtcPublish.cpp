...
void YangRtcPublish::run() {
	...
	while (m_isInLoop == 1) {
		...
		if (m_audioBuffer && m_audioBuffer->size() > 0) {
			...
			if (m_peerConn->on_audio(data.getAudioFrame(data.context)) != Yang_Ok) {
				m_peerConn->close();
				continue;
			}
		}
		...
		if (m_videoBuffer && m_videoBuffer->size() > 0) {
			...
			if (videoFrame.frametype == YANG_Frametype_I) {
                ...
                m_peerConn->on_video(data.getVideoFrame(data.context));
			...
			if (m_peerConn->on_video(data.getVideoFrame(data.context)) != Yang_Ok) {
				m_peerConn->close();
			}
		}		
	}
